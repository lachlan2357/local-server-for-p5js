...
app.get("/:username/sketches/:sketch_id/:file_path(*)", async (req, res) => {
    const { username, sketch_id, file_path } = req.params;
    await ensure_exists(username, sketch_id);
    const content = await get_file(username, sketch_id, file_path ?? "index.html");
    if (content === undefined)
        res.status(404).send(`File '${file_path} could not be found for project '${sketch_id}' by '${username}'.`);
    else
        res.send(content);
});
